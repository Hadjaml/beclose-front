...
**Dernière mise à jour** : 2026-09-23 · **Par** : Antigravity (décision Rochinel 8375ac7e)
...
## Inversion du sens du contexte — .agentmesh source de vérité (2026-09-23)

- **Décision de Rochinel (23/09/2026, décision `8375ac7e`)** : inversion du sens du contexte. Plusieurs agents de runtimes différents (Claude, Codex, Antigravity, OpenCode...) se succèdent sur ce dépôt. Le contexte ne doit dépendre d'aucun agent ni d'aucun runtime. La seule source de vérité sur ce dépôt est désormais `.agentmesh/skills/project/`.
- **Résolution du conflit d'ordre de lecture** : l'ancien conflit d'ordre de lecture est définitivement résolu. `.claude/skills/bewise-app/` est réduit à de simples pointeurs vers `AGENTS.md` et `.agentmesh/skills/project/`.
- **Contenu déplacé** :
  - `SKILL.md` : protocole obligatoire, posture, coordination AgentMesh, lien Beclose avec table de correspondance, conteneurisation et index des références.
  - `avancement.md` : historique complet des chantiers (containerisation, CI, auth, endpoints v0, pagination, schémas ICP/BANT, adaptations wire) et état courant.
  - `conventions.md` : pratiques stables et pièges documentés (Next.js cross-origin, Git, Docker, CI, API v0).
- **Vérifié réellement** :
  - `agentmesh skill validate --path .` : 4/4 fichiers canoniques valides.
  - `git diff --check` : 0 anomalie d'espace ou conflit.
  - Résolution des liens relatifs : vérifiés avec `test -r`.
